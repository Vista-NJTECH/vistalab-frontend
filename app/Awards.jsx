...
function YearCard({ item, currentIndexState, clickCallback }) {
  const commonStyle =
  ...
  return (
    <div
      ...
      className={
        currentIndexState === item
          ? `bg-slate-200 border-theme-dark ${commonStyle}`
          : `border-theme-light  ${commonStyle}`
      }
    >
      <span>{item}</span>
      {currentIndexState === item && (
      ...
      <div className='w-1/3 flex flex-col items-center justify-start gap-7 rounded-r-none rounded-xl pl-16 py-10'>
        ...
        {yearsData.map((item) => (
          <YearCard
            ...
            clickCallback={() => setCurrentIndex(item)}
          />
        ))}
      </div>
