...
async function StudyCard({ category }) {
  const res = await fetch(`http://124.223.196.177:8181/study/getall?class=${category.path}`);
  if (!res.ok) throw new Error("Failed to fetch data");
  const data = await res.json();
  const categories = [...new Set(data.data.map((item) => item.coursename))];
  return (
    <div className='flex flex-col gap-1 p-2 shadow-md text-gray-400 border-2 border-slate-200 rounded-md'>
      ...
      <div className='flex flex-col gap-1'>
        {categories.map((item, index) => (
          <Link key={index} href={"/study/" + category.title + "/" + item} className='text-slate-800 hover:text-theme'>
            {item}
          </Link>
        ))}
      </div>
