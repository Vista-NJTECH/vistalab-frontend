    ...
    <div className='px-48 flex flex-col items-center justify-center py-20 gap-5'>
      ...
      <div className='flex flex-col gap-3 items-center justify-center'>
        <div className='w-[700px] h-[500px] relative group border-8 border-[#cdaa7d] border-dashed p-2'>
          ...
          <div className='group-hover:block hidden absolute top-[50%] -translate-x-0 -translate-y-[-50%] left-5 text-2xl rounded-full cursor-pointer text-white bg-black/30'>
            <MdKeyboardArrowLeft onClick={prevSlide} size={30} />
          </div>
          <div className='group-hover:block hidden absolute top-[50%] -translate-x-0 -translate-y-[-50%] right-5 text-2xl rounded-full cursor-pointer text-white bg-black/30'>
            <MdKeyboardArrowRight onClick={nextSlide} size={30} />
          </div>
        </div>
        <div className='flex flex-row gap-2'>
          {images.map((item, index) => (
            <div key={index} className='cursor-pointer text-slate-700' onClick={() => setCurrentSlideIndex(index)}>
